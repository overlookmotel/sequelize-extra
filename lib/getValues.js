...
module.exports = {
	...
	reverseIncludes: function(item, modelChain) {
		...
		for (var i = 1; i < modelChain.length; i++) {
			...
			var thisItem = item[thisModel.dataNamePlural][0];
			delete item.dataValues[thisModel.dataNamePlural];
			delete item[thisModel.dataNamePlural];
			
			if (!thisItem) break;
			
			if (!thisItem[model.dataName]) {
				thisItem.dataValues[model.dataName] = item;
				thisItem[model.dataName] = item;
			}
